...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
    return (
        <div className="space-y-2">
            <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-on-tertiary-container-stitch px-1">{label}</label>
            {children}
            {hint && <p className="text-[11px] text-secondary-stitch/60 font-medium px-1 italic">{hint}</p>}
        </div>
    );
}
...
function Section({ title, accent, children }: { title: string; accent?: "amber" | "rose"; children: React.ReactNode }) {
    ...
    return (
        <div className="space-y-6 p-6 bg-white rounded-[28px] border border-outline-variant-stitch/10 shadow-glow-stitch">
            <div className="flex items-center gap-3">
                <span className={`w-2 h-2 rounded-full ${dot} shadow-lg`} />
                <h3 className="font-black text-primary-stitch text-[11px] uppercase tracking-[0.15em]">{title}</h3>
            </div>
...
function SortableTabItem({ tab, isActive, isLocked, isVisible, canDrag, onSelect, onToggle }: {
    ...
    return (
        <div
            ...
            className={`flex items-center relative transition-all duration-300 ${isActive ? "bg-on-primary-container-stitch/5" : "bg-transparent"} ${isDragging ? "opacity-50 z-50 scale-105" : ""}`}
        >
            {/* Drag handle */}
            {canDrag ? (
                <button
                    {...attributes}
                    {...listeners}
                    className="pl-4 pr-1 py-4 text-on-tertiary-container-stitch/30 hover:text-primary-stitch cursor-grab active:cursor-grabbing touch-none flex-shrink-0"
                >
                    <GripVertical className="w-4 h-4" />
                </button>
            ) : (
                <span className="pl-4 pr-1 py-4 w-9 flex-shrink-0" />
            )}

            {/* Tombol tab utama */}
            <button
                onClick={onSelect}
                className={`flex-1 flex items-center gap-3 py-4 pr-3 transition-all text-left relative ${isActive
                    ? "text-primary-stitch"
                    : "text-secondary-stitch/60 hover:text-primary-stitch"
                    }`}
            >
                {isActive && (
                    <span className="absolute left-[-2px] top-1/2 -translate-y-1/2 w-1 h-6 bg-primary-stitch rounded-r-full" />
                )}
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-primary-stitch" : "text-secondary-stitch/40"}`} />
                <span className={`truncate text-[11px] font-black uppercase tracking-wider ${isActive ? "opacity-100" : "opacity-70"}`}>{tab.label}</span>
            </button>
            ...
            {onToggle && !isLocked && (
                ...
                >
                    <span className={`w-8 h-4.5 rounded-full relative flex items-center transition-all duration-300 shadow-inner ${isVisible ? "bg-tertiary-stitch" : "bg-surface-container-high-stitch"
                        }`}>
                        <span className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full shadow-lg transition-transform duration-300 ${isVisible ? "translate-x-4" : "translate-x-0.5"
                            }`} />
                    </span>
                </button>
            )}
            ...
            {isLocked && (
                <span className="pr-4 flex-shrink-0" title="Wajib tampil">
                    <Lock className="w-3 h-3 text-secondary-stitch/20" />
                </span>
            )}
        </div>
    );
...
export default function EditorClient({ initialData }: EditorClientProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-surface-stitch min-h-screen selection:bg-tertiary-fixed-dim-stitch font-sans">

            {/* ── Sticky Header ── */}
            <div className="sticky top-0 z-40 bg-white/70 backdrop-blur-2xl border-b border-outline-variant-stitch/10 px-6 py-4">
                <div className="max-w-full mx-auto flex flex-col sm:flex-row gap-4 items-center justify-between">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                        <Link href={`/dashboard/undangan/${initialData.id}`} className="p-2 rounded-full text-secondary-stitch hover:text-primary-stitch hover:bg-surface-container-low-stitch transition-all active:scale-90">
                            <ArrowLeft className="w-5 h-5" />
                        </Link>
                        <div>
                            <span className="text-[10px] font-black tracking-[0.2em] uppercase text-on-tertiary-container-stitch block mb-0.5">Editor</span>
                            <h1 className="font-black text-xl text-primary-stitch tracking-tighter line-clamp-1 truncate max-w-[200px] md:max-w-xs leading-none">
                                {coupleName}
                            </h1>
                        </div>
                    </div>
                    <div className="flex w-full sm:w-auto items-center gap-3">
                        {/* Mobile toggle */}
                        <div className="md:hidden flex rounded-full border border-outline-variant-stitch/20 bg-surface-container-low-stitch p-1 flex-shrink-0 shadow-inner">
                            <button
                                type="button"
                                onClick={() => setMobileMode("edit")}
                                className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${mobileMode === "edit"
                                    ? "bg-primary-stitch text-white shadow-lg"
                                    : "text-secondary-stitch/60"
                                    }`}
                            >
                                Edit
                            </button>
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${mobileMode === "preview"
                                    ? "bg-primary-stitch text-white shadow-lg"
                                    : "text-secondary-stitch/60"
                                    }`}
                            >
                        ...
                        <button
                            className="hidden md:flex h-11 px-6 rounded-full border border-outline-variant-stitch/30 text-primary-stitch bg-white hover:bg-surface-container-low-stitch transition-all active:scale-95 text-xs font-bold items-center gap-2"
                        ...
                        <button
                            ...
                            disabled={isSaving}
                            className="flex-1 sm:flex-none h-11 px-8 rounded-full bg-primary-stitch text-white shadow-xl shadow-primary-stitch/20 hover:scale-105 hover:rotate-1 transition-all active:scale-95 font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2"
                        >
                            {isSaving
            ...
            {/* ── Split Pane Body ── */}
            <div className="flex flex-col md:flex-row flex-1" style={{ minHeight: "calc(100vh - 80px)" }}>

                {/* LEFT — Editor Form */}
                <div className={`flex flex-col overflow-hidden bg-surface-stitch transition-all duration-300 md:sticky md:top-20 md:h-[calc(100vh-80px)]
    ${mobileMode === "preview" ? "hidden md:flex" : "flex"}
    ${showPreview ? "w-full md:w-[440px] lg:w-[480px] md:flex-shrink-0" : "w-full"}
`}>

                    {/* Tab Nav */}
                    <div className="flex md:flex-row overflow-x-auto md:overflow-visible flex-shrink-0 border-b border-outline-variant-stitch/10 bg-white">
                        <nav className="flex md:flex-col w-full md:w-auto md:border-r md:border-outline-variant-stitch/10 md:bg-surface-container-low-stitch overflow-x-auto">
                            ...
                            <div className="md:hidden flex gap-1 px-3 py-2.5 overflow-x-auto scrollbar-hide">
                                {tabs.map((tab) => {
                                    ...
                                    const canReorder = !!sectionId && !NO_REORDER_TABS.includes(tab.id);
                                            ...
                                            {canReorder && (
                                                <div className="flex flex-col border-r border-white/20">
                                                    <button
                                                        ...
                                                        disabled={currentIdx <= firstMovableIdx}
                                                        className="px-1.5 py-1 hover:bg-white/10 disabled:opacity-20 transition-colors"
                                                    >
                    ...
                    <div className="flex flex-1 overflow-hidden">
                        ...
                        <nav className="hidden md:flex flex-col w-52 flex-shrink-0 border-r border-outline-variant-stitch/10 bg-surface-container-low-stitch overflow-y-auto py-4">
                            <DndContext
                                sensors={useSensors(
                                    ...
                                    {tabs.map((tab) => {
                                        ...
                                        const isVisible = sectionId
                                            ? (sectionId in (formData.sections_visibility as any)
                                                ...
                                                : true)
                                            : null;
                                        const canDrag = !!sectionId && !NO_REORDER_TABS.includes(tab.id) && tab.id !== "infodasar";
                                ...
                                {activeTab === "infodasar" && (
                                    <div className="space-y-6 animate-in fade-in duration-200">
                                        ...
                                        <Field label="URL Undangan Publik">
                                            <div className="flex items-center gap-3">
                                                <div className="flex-1 bg-surface-container-high-stitch text-primary-stitch text-xs font-black tracking-wider px-4 py-3.5 rounded-2xl border border-outline-variant-stitch/20 font-mono truncate shadow-inner">
                                                    undang.io/invite/{formData.slug}
                                                </div>
                                                <button
                                                    className="p-3.5 rounded-2xl bg-white border border-outline-variant-stitch/30 text-primary-stitch hover:bg-surface-container-low-stitch transition-all active:scale-95 shadow-sm"
                                                    title="Salin Link"
                                ...
                                {activeTab === "fotocover" && (
                                    <div className="space-y-5 animate-in fade-in duration-200">
                                        ...
                                        {formData.couple_photo_url && (
                                            <div className="relative w-full rounded-[32px] overflow-hidden border-2 border-outline-variant-stitch/20 shadow-2xl group/img">
                                                <Image
                                                    src={formData.couple_photo_url}
                                                    alt="Foto pasangan"
                                                    width={480} height={280}
                                                    className="w-full object-cover max-h-64 transition-transform duration-1000 group-hover/img:scale-110"
                                                    unoptimized={formData.couple_photo_url.startsWith("http://localhost") || formData.couple_photo_url.startsWith("/uploads")}
                                                />
                                                <div className="absolute inset-0 bg-black/20 group-hover/img:bg-black/40 transition-colors duration-500" />
                                                <button
                                                    onClick={() => handleChange("couple_photo_url", "")}
                                                    className="absolute top-4 right-4 w-10 h-10 bg-white/10 backdrop-blur-md hover:bg-error-stitch text-white rounded-full flex items-center justify-center transition-all shadow-xl active:scale-90"
                                                >
                                                    <X className="w-4 h-4" />
                                                </button>
                                            </div>
                                        ...
                                        <div
                                            ...
                                            className={`flex flex-col items-center justify-center gap-4 p-12 rounded-[32px] border-2 border-dashed transition-all duration-500 cursor-pointer ${isUploading
                                        ...
                                        >
                                            {isUploading ? (
                                                <>
                                                    <Loader2 className="w-10 h-10 text-tertiary-stitch animate-spin" />
                                                    <p className="text-[10px] font-black uppercase tracking-widest text-on-tertiary-container-stitch">Processing Portfolio...</p>
                                                </>
                                            ) : (
                                                <>
                                                    <div className="w-16 h-16 bg-white rounded-[24px] shadow-glow-stitch flex items-center justify-center mb-2">
                                                        <Upload className="w-8 h-8 text-primary-stitch" />
                                                    </div>
                                                    <div className="text-center">
                                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-stitch mb-1">
                                                            {formData.couple_photo_url ? "Update Visuals" : "Upload Feature Image"}
                                                        </p>
                                                        <p className="text-[10px] text-secondary-stitch/60 font-medium">MAX. 5MB · JPG, PNG, WEBP</p>
                                                    </div>
                                                </>
                                            )}
                                            <input
                ...
                {(showPreview || mobileMode === "preview") && (
                    <div className="flex flex-col flex-1 overflow-hidden border-t md:border-t-0 md:border-l border-outline-variant-stitch/10 bg-surface-stitch min-h-[600px] md:min-h-0">
                        <div className="flex items-center gap-3 px-6 py-3 bg-white border-b border-outline-variant-stitch/10 flex-shrink-0">
                            <div className="flex gap-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-error-stitch shadow-lg shadow-error-stitch/20" />
                                <span className="w-2.5 h-2.5 rounded-full bg-tertiary-stitch shadow-lg shadow-tertiary-stitch/20" />
                                <span className="w-2.5 h-2.5 rounded-full bg-primary-stitch/20 shadow-lg" />
                            </div>
                            <div className="flex-1 mx-4 text-center">
                                <span className="text-[10px] font-black tracking-widest uppercase text-secondary-stitch/40 bg-surface-container-low-stitch px-4 py-1.5 rounded-full shadow-inner border border-outline-variant-stitch/5">
                                    undang.io/invite/{formData.slug}
                                </span>
                            </div>
                            <span className="text-[9px] font-black tracking-tighter text-on-tertiary-container-stitch bg-tertiary-fixed-dim-stitch px-3 py-1 rounded-full shadow-lg">
                                PREVIEWING TITANIUM
                            </span>
                        </div>
                        <div className="flex-1 overflow-y-auto">
